...
class LicensePlateDatasetCreator:
    def __init__(self, 
        ...
        # Set default paths using Path
        project_root = Path(__file__).parent
        self.source_dir = Path(source_dir) if source_dir else project_root / "data" / "our database"
        self.output_dir = Path(output_dir) if output_dir else project_root / "data" / "VALID_PLATES"
        self.yolo_weights_path = Path(yolo_weights_path) if yolo_weights_path else project_root / "yolo_v8_custom_updated" / "weights" / "best.pt"
        
        self.crops_dir = self.output_dir / "plate_crops"
        ...
        self.yolo_model = load_yolo_model(str(self.yolo_weights_path))
